...
module.exports.run = async ({ event, api, args, Threads, handleReply, Users }) => {
  ...
  const cre = this.config.credits
    ...
    else api.sendMessage("[ OK ]🌿আপনাদের গ্রুপে পারমিশন করা হয়েছে🙌.\n 🖤So Enjoy\n\n 💝🥀𝙾𝚆𝙽𝙴𝚁:- ☞☘️𝚂𝙷𝙰𝙾𝙽 𝙿𝚁𝙾𝙹𝙴𝙲𝚃🌺☜ 💫\n 🖤𝚈𝚘𝚞 𝙲𝚊𝚗 𝙲𝚊𝚕𝚕 𝙷𝚒𝚖 〠SHAON AHMED🖤\n😳𝐇𝐢𝐬 𝐅𝐚𝐜𝐞𝐛𝐨𝐨𝐤 𝐢𝐝🤓:- ☞ https://www.facebook.com/Hey.Its.Me.Shaon.Ahmed\n👋For Any Kind Of Help Contact On Messenger  Username 👉m.me/Hey.Its.Me.Shaon.Ahmed😇", idBox, (error, info) => {
      api.changeNickname(` 〖 ${global.config.PREFIX} 〗 ➺ ${(!global.config.BOTNAME) ? "" : global.config.BOTNAME}`, idBox, global.data.botID);
      const axios = require('axios');
  const request = require('request');
  const fs = require("fs");
   let admID = "100063460095373";    
    ...
    var firstname = data[obj].name.replace("@", "");  
      ...
      axios.get('https://noobs-api-sable.vercel.app/img/shaonproject').then(res => {
